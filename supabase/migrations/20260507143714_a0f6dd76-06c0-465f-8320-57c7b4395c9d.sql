...
-- Allow admins to cancel orders containing random filament items
...
BEGIN
  IF NEW.status = 'cancelled' AND OLD.status <> 'cancelled' THEN
    IF EXISTS (SELECT 1 FROM public.random_filament_orders WHERE order_id = NEW.id)
...
-- Update cancel_order to also clean up random_filament_orders rows when cancelled
...
BEGIN
  ...
  -- Clean up random_filament_orders rows tied to this order (stock was already restored above
  -- because order_items were updated to the revealed product/option/color by finalize_and_reveal_rf_for_order)
  BEGIN
    DELETE FROM public.random_filament_orders WHERE order_id = p_order_id;
