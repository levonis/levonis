...
BEGIN
  ...
  IF v_settings.season_ends_at IS NOT NULL AND v_settings.season_ends_at <= now() THEN
    ...
    UPDATE crossy_road_high_scores
      ...
      WHERE true;
    UPDATE crossy_road_settings SET season_ends_at = NULL, season_starts_at = NULL, updated_at = now() WHERE true;
    v_total := v_total + v_awarded;
    v_results := v_results || jsonb_build_object('game', 'crossy_road', 'awarded', v_awarded);
  END IF;

  -- STACK TOWER (uses achieved_at, not updated_at)
  ...
  IF v_settings.season_ends_at IS NOT NULL AND v_settings.season_ends_at <= now() THEN
    ...
    UPDATE stack_game_high_scores
      SET all_time_high_score = GREATEST(COALESCE(all_time_high_score, 0), COALESCE(high_score, 0)),
          high_score = 0, season = v_season + 1, achieved_at = now()
      WHERE true;
    UPDATE stack_game_settings SET season_ends_at = NULL, season_starts_at = NULL, updated_at = now() WHERE true;
    v_total := v_total + v_awarded;
  ...
  IF v_settings.season_ends_at IS NOT NULL AND v_settings.season_ends_at <= now() THEN
    ...
    UPDATE knife_rain_high_scores
      ...
      WHERE true;
    UPDATE knife_rain_settings SET season_ends_at = NULL, season_starts_at = NULL, updated_at = now() WHERE true;
    v_total := v_total + v_awarded;
