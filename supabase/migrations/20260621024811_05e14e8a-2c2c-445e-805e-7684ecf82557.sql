-- 1) New product columns for land shipping
ALTER TABLE public.products
  ADD COLUMN IF NOT EXISTS land_price numeric NULL,
  ADD COLUMN IF NOT EXISTS commission_land_iqd numeric NOT NULL DEFAULT 0;

-- Mirror visibility on the products_admin view by recreating it (preserve grants below)
-- We don't redefine the view here because it's auto-defined elsewhere; new columns
-- on the base table are surfaced through `SELECT *` views automatically. If the
-- products_admin view uses explicit columns it will need a follow-up; the
-- recompute function uses the base table directly so server-side pricing is unaffected.

-- 2) New shipping_settings rows for land shipping (idempotent)
INSERT INTO public.shipping_settings (setting_key, setting_value, description_ar) VALUES
  ('land_price_per_kg_usd', 4, 'سعر الشحن البري لكل كيلوغرام (بالدولار)')
ON CONFLICT (setting_key) DO NOTHING;

-- 3) Legacy data migration: convert 'both' -> 'sea,air' so all code paths can
-- treat shipping_type as a comma-separated token list.
UPDATE public.products
   SET shipping_type = 'sea,air'
 WHERE shipping_type = 'both';

-- 4) Recompute function — add land branch and switch sea/air checks to token list
...
BEGIN
  ...
  -- Parse shipping_type tokens (supports legacy 'sea'/'air'/'both' and new 'sea,air,land')
  ...
  -- Preference order for direct-sale shipping fallback: sea > air > land
  ...
  IF v_land_price   IS NOT NULL THEN v_prices := array_append(v_prices, v_land_price);   END IF;
...
-- Recompute everything once so new land_price is populated
