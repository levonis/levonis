-- 1. Add product_weights JSONB column
ALTER TABLE public.random_filament_offers
  ADD COLUMN IF NOT EXISTS product_weights jsonb NOT NULL DEFAULT '{}'::jsonb;

-- 2. Helper: weighted random pick from a jsonb array of {key, weight}
CREATE OR REPLACE FUNCTION public._rf_weighted_pick(p_items jsonb)
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_items) LOOP
    v_total := v_total + GREATEST((v_item->>'w')::double precision, 0.0001);
  END LOOP;
  IF v_total <= 0 THEN RETURN NULL; END IF;
  v_r := random() * v_total;
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_items) LOOP
    v_w := GREATEST((v_item->>'w')::double precision, 0.0001);
    v_acc := v_acc + v_w;
    IF v_acc >= v_r THEN
      RETURN v_item->>'k';
    END IF;
  END LOOP;
  -- fallback (last)
  RETURN (SELECT (value->>'k') FROM jsonb_array_elements(p_items) ORDER BY ((value->>'k')) DESC LIMIT 1);
END;
$$;

-- 3. Rewrite create_random_filament_order with weighted picks via _rf_weighted_pick
...
BEGIN
  ...
  IF v_sale_type = 'direct' THEN
    ...
    -- Color pool: key = color index (so we can locate it after)
    ...
    -- Option key pool inside this color
    SELECT jsonb_agg(jsonb_build_object(
             'k', kv.key,
  ...
  ELSE
    ...
    IF v_pick IS NOT NULL THEN
      v_color := (v_product.colors::jsonb) -> (v_pick::int);
    END IF;

    -- Weighted option pick (preorder) from product_options
    SELECT jsonb_agg(jsonb_build_object(
             'k', po.id::text,
