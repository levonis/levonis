...
-- Fix the prevent_ticket_fraud trigger to allow SECURITY DEFINER functions
-- The issue: purchase_product_offer is SECURITY DEFINER but the trigger still blocks it
-- Solution: Use a session variable that SECURITY DEFINER functions set before modifying tickets

-- 1. Update purchase_product_offer to set a flag before modifying tickets
...
BEGIN
  ...
  IF total_tickets > 0 THEN
    -- Set flag to allow the fraud trigger to pass
...
-- 2. Update the fraud prevention trigger to check for the bypass flag
