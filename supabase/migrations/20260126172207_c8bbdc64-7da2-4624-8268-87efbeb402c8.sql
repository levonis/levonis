...
-- Update the handle_new_user function to fail if username is already taken
...
BEGIN
  ...
  IF provided_username IS NOT NULL AND provided_username != '' THEN
    ...
    ELSE
      -- Username exists - still use it but with suffix to avoid blocking registration
      -- The frontend already checks for uniqueness, so this is a race condition fallback
