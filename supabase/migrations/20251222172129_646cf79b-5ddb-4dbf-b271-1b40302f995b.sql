...
BEGIN
  ...
  -- Generate a single random value from 0 to 100
  random_val := random() * 100;

  -- The probability distribution is:
  -- [0 to better_luck_prob) = better luck
  -- [better_luck_prob to better_luck_prob + letter1_prob) = letter 1
  -- [better_luck_prob + letter1_prob to better_luck_prob + letter1_prob + letter2_prob) = letter 2
  -- etc.
  -- If total (better_luck_prob + all letter probs) < 100, the remainder is also better luck

  IF random_val < better_luck_prob THEN
    -- Hit better luck zone
    is_better_luck := true;
    awarded_letter := NULL;
  ELSIF letter_probs IS NULL THEN
    -- No probabilities defined - equal distribution among letters
    awarded_letter := available_letters[1 + floor(random() * array_length(available_letters, 1))::int];
  ELSIF total_letter_prob <= 0 THEN
    -- All letters have 0 probability
    is_better_luck := true;
    awarded_letter := NULL;
  ELSE
  ...
  IF NOT is_better_luck AND awarded_letter IS NOT NULL THEN
