CREATE OR REPLACE FUNCTION public.check_stack_milestone(p_user_id uuid, p_score integer, p_session_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    IF v_milestone.product_id IS NOT NULL THEN
      IF NOT public.deduct_prize_stock(v_milestone.product_id) THEN
        CONTINUE;
      END IF;
