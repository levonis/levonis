-- First, create a table for letter prize discount codes if it doesn't exist
CREATE TABLE IF NOT EXISTS public.letter_prize_coupons (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL,
  competition_id UUID NOT NULL REFERENCES public.competitions(id) ON DELETE CASCADE,
  redemption_id UUID NOT NULL REFERENCES public.letter_prize_redemptions(id) ON DELETE CASCADE,
...
-- Enable RLS
ALTER TABLE public.letter_prize_coupons ENABLE ROW LEVEL SECURITY;

-- Create policies
CREATE POLICY "Users can view their own coupons" 
ON public.letter_prize_coupons 
FOR SELECT 
...
CREATE POLICY "Require authentication for coupons" 
ON public.letter_prize_coupons 
FOR ALL 
USING (auth.uid() IS NOT NULL)
WITH CHECK (auth.uid() IS NOT NULL);

-- Drop and recreate the redeem function with coupon generation
...
BEGIN
  ...
  SELECT 
    ...
    'المستخدم أكمل كلمة "' || p_word || '" وربح: ' || v_prize_name || '. كود الخصم: ' || v_coupon_code,
    'info'
  FROM user_roles ur
  WHERE ur.role = 'admin';
