CREATE OR REPLACE FUNCTION public.check_stack_milestone(p_user_id uuid, p_score integer, p_session_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    IF v_milestone.product_id IS NOT NULL THEN
      IF v_milestone.selected_option_id IS NOT NULL THEN
        ...
        WHERE id = v_milestone.selected_option_id;
      END IF;
    ...
    BEGIN
      ...
      VALUES (v_milestone.id, p_user_id, v_session_uuid, p_score);
    EXCEPTION
  ...
  RETURN jsonb_strip_nulls(jsonb_build_object(
    ...
    'target_score', (v_primary_prize->>'target_score')::integer,
    'prize_name', v_primary_prize->>'prize_name',
    'prize_image', v_primary_prize->>'prize_image',
    'stock_remaining', CASE WHEN v_primary_prize ? 'stock_remaining' THEN (v_primary_prize->>'stock_remaining')::integer ELSE NULL END,
    'won_prizes', v_won_prizes,
    'won_count', v_prizes_count
  ));
END;
$function$;