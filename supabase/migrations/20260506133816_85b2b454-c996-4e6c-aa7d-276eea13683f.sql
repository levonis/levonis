CREATE OR REPLACE FUNCTION public._admin_filtered_payload(_table_name text, _payload jsonb, _exclude text[] DEFAULT ARRAY['id','created_at']::text[])
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  ...
  FROM jsonb_each(COALESCE(_payload, '{}'::jsonb)) AS e(key, value)
  WHERE e.key IN (
    SELECT c.column_name
    FROM information_schema.columns c
    WHERE c.table_schema = 'public'
      AND c.table_name = _table_name
      AND NOT (c.column_name = ANY(_exclude))
  );
$$;
...
BEGIN
  ...
  SELECT string_agg(format('%1$I = (r).*::public.orders.%1$I', col), ', ')
  INTO set_clause
...
BEGIN
  ...
  SELECT string_agg(format('%1$I = (r).*::public.products.%1$I', col), ', ')
  INTO set_clause
  FROM unnest(cols) AS col;
...
REVOKE ALL ON FUNCTION public._admin_filtered_payload(text, jsonb, text[]) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.admin_update_order(uuid, jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.admin_create_order(jsonb) FROM PUBLIC;
