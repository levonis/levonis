  ...
  LOOP
    ...
    FOR v_unit IN 1..v_oi.quantity LOOP
      ...
      IF v_offer.sale_type = 'direct' THEN
        ...
        SELECT * INTO v_option FROM public.product_options WHERE id = v_option_key::uuid;
...
-- Backfill the affected order item
UPDATE public.order_items oi
SET selected_option = COALESCE(po.name_ar, po.name, oi.selected_option)
FROM public.product_options po
WHERE oi.product_option_id = po.id
  AND oi.rf_offer_id IS NOT NULL
  AND (oi.selected_option IS NULL OR oi.selected_option = '');