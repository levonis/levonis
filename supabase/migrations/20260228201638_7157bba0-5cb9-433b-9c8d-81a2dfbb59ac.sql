...
-- Fix 1: Update cancel_order to use selected_option as fallback for option name
CREATE OR REPLACE FUNCTION public.cancel_order(p_order_id text, p_cancelled_by text DEFAULT 'customer')
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_order.order_type = 'direct' AND COALESCE(v_order.stock_deducted, false) IS TRUE THEN
    ...
    LOOP
      ...
      -- Get option name: from product_options table OR from selected_option field
      v_option_name := NULL;
      IF v_item.product_option_id IS NOT NULL THEN
        ...
        WHERE id = v_item.product_option_id;
      END IF;
      -- Fallback to selected_option if product_option_id is NULL
...
-- Fix 2: Update deduct_order_stock to use selected_option as fallback
...
-- Fix 3: Update sold_count trigger to include processing and on_the_way statuses
    ...
    IF affected_product_ids IS NOT NULL THEN
      ...
      SET sold_count = COALESCE((
        ...
        WHERE oi.product_id = p.id
          AND o.status IN ('confirmed', 'processing', 'arrived_warehouse', 'shipped', 'on_the_way', 'arrived_iraq', 'delivered')
      ), 0)
