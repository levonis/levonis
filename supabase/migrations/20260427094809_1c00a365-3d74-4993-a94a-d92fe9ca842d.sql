...
BEGIN
  ...
  IF v_settings.season_ends_at IS NOT NULL AND v_settings.season_ends_at <= now() THEN
    ...
    UPDATE crossy_road_high_scores
      ...
      WHERE true;
    -- Only clear schedule if admin has NOT pre-scheduled a future season.
    UPDATE crossy_road_settings
      SET season_ends_at = NULL,
          season_starts_at = NULL,
          updated_at = now()
      WHERE (season_starts_at IS NULL OR season_starts_at <= now())
        AND (season_ends_at IS NULL OR season_ends_at <= now());
