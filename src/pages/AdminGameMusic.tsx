...
import AdminLayout, { AdminSection, AdminEmptyState, AdminLoading } from "@/components/admin/AdminLayout";
...
export default function AdminGameMusic() {
  ...
  const openEdit = (station: MusicStation) => {
    setEditing(station);
    setNameAr(station.name_ar);
    setUploadedUrl(station.file_url);
    setDialogOpen(true);
  };
  ...
  const handleSave = () => {
    if (!nameAr.trim() || !uploadedUrl) {
      toast.error("يرجى إدخال الاسم ورفع ملف الموسيقى");
      return;
    }
    saveMutation.mutate({
      id: editing?.id,
      ...
      display_order: stations?.length || 0,
      is_active: true,
    });
  };
  ...
  return (
    <AdminLayout
      title="إعدادات موسيقى الألعاب"
      description="إدارة محطات الموسيقى في صفحة الألعاب"
    ...
    >
      {isLoading ? (
        ...
        <AdminSection title={`المحطات (${stations.length})`}>
          <div className="space-y-3">
            {stations.map((station, idx) => (
              <div
                key={station.id}
                className="flex items-center gap-3 p-4 rounded-xl border border-border/50 bg-card hover:border-border transition-colors"
              >
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => toggleMutation.mutate({ id: station.id, is_active: checked })}
                />
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent dir="rtl" className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{editing ? "تعديل المحطة" : "إضافة محطة جديدة"}</DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4 pt-2">
              ...
              {uploadedUrl ? (
                ...
                <label className="flex flex-col items-center gap-2 p-6 rounded-lg border-2 border-dashed border-border/60 hover:border-primary/50 cursor-pointer transition-colors bg-muted/20">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {uploading ? "جاري الرفع..." : "اضغط لرفع ملف صوتي (MP3, WAV, OGG)"}
                  </span>
            ...
            <div className="flex gap-2 justify-end pt-2">
              ...
              <Button
                onClick={handleSave}
                disabled={saveMutation.isPending || !nameAr.trim() || !uploadedUrl}
              >
