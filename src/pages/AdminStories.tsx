...
import { Plus, Pencil, Trash2, GripVertical, Film, ArrowRight, Upload, Loader2, X, Eye, EyeOff, PlayCircle, Clapperboard } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { cn } from '@/lib/utils';

const AdminReelsManager = lazy(() => import('@/components/admin/AdminReelsManager'));

type TabType = 'stories' | 'reels';
...
export default function AdminStories() {
  ...
  const tabItems = [
    { value: 'stories' as TabType, icon: PlayCircle, label: 'ستوريات' },
    { value: 'reels' as TabType, icon: Clapperboard, label: 'ريلز' },
  ];
      ...
      <div className="container mx-auto px-4 py-6 max-w-5xl">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground">إدارة المحتوى المرئي</h1>
              <p className="text-sm text-muted-foreground">ستوريات وريلز الصفحة الرئيسية</p>
            </div>
          </div>
        </div>
...
/* ───────────── Stories Manager (extracted from old AdminStories) ───────────── */
...
function StoriesManager() {
  ...
  const { data: sections = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('story_sections').select('*').order('display_order', { ascending: true });
  ...
  const { data: videos = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.from('story_videos').select('*').eq('section_id', selectedSectionId).order('display_order', { ascending: true });
      if (error) throw error;
      return data;
    },
  ...
  const openEditSection = (section: any) => {
    ...
    setSectionForm({ title_ar: section.title_ar, thumbnail_url: section.thumbnail_url || '', display_order: section.display_order, is_active: section.is_active });
  ...
  const openEditVideo = (video: any) => {
    ...
    setVideoForm({ video_url: video.video_url, duration_seconds: video.duration_seconds || 0, display_order: video.display_order, is_active: video.is_active });
    setVideoDialog(true);
  };
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-bold text-foreground">أقسام الستوريات</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="space-y-2">
          ...
          {isLoading ? (
            ...
            sections.map((s) => (
              <Card key={s.id} className={`cursor-pointer transition-colors ${selectedSectionId === s.id ? 'border-primary bg-primary/5' : 'hover:border-primary/30'}`} onClick={() => setSelectedSectionId(s.id)}>
                <CardContent className="p-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <Badge variant={s.is_active ? 'default' : 'secondary'} className="text-[10px] h-4">{s.is_active ? 'نشط' : 'متوقف'}</Badge>
                      <span className="text-[10px] text-muted-foreground">ترتيب: {s.display_order}</span>
                    </div>
                  </div>
                  <div className="flex gap-1">
                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={(e) => { e.stopPropagation(); openEditSection(s); }}><Pencil className="h-3 w-3" /></Button>
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={(e) => { e.stopPropagation(); if (confirm('حذف هذا القسم وجميع فيديوهاته؟')) deleteSectionMutation.mutate(s.id); }}><Trash2 className="h-3 w-3" /></Button>
                  </div>
                </CardContent>
              </Card>
            ))
            ...
            <Card>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  <Button onClick={openNewVideo} size="sm" variant="outline"><Plus className="h-3 w-3 ml-1" /> إضافة فيديو</Button>
                </div>
              </CardHeader>
              <CardContent>
                {videos.length === 0 ? (
                    ...
                    {videos.map((v) => (
                      <div key={v.id} className="relative group rounded-xl overflow-hidden bg-muted aspect-[9/16]">
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                          <Button variant="ghost" size="icon" className="h-8 w-8 bg-white/20 text-white" onClick={() => openEditVideo(v)}><Pencil className="h-3 w-3" /></Button>
                          <Button variant="ghost" size="icon" className="h-8 w-8 bg-white/20 text-white" onClick={() => { if (confirm('حذف هذا الفيديو؟')) deleteVideoMutation.mutate(v.id); }}><Trash2 className="h-3 w-3" /></Button>
                        </div>
                        <div className="absolute bottom-1 left-1 right-1 flex items-center justify-between">
                          <Badge variant={v.is_active ? 'default' : 'secondary'} className="text-[9px] h-4">{v.is_active ? <Eye className="h-2.5 w-2.5" /> : <EyeOff className="h-2.5 w-2.5" />}</Badge>
      ...
      <Dialog open={sectionDialog} onOpenChange={setSectionDialog}>
        <DialogContent dir="rtl" className="max-w-md">
          <DialogHeader><DialogTitle>{editingSection ? 'تعديل القسم' : 'قسم جديد'}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div><Label>الاسم</Label><Input value={sectionForm.title_ar} onChange={(e) => setSectionForm((p) => ({ ...p, title_ar: e.target.value }))} placeholder="مثلاً: عروض اليوم" /></div>
            <div>
              ...
              <div className="flex gap-2 items-center mt-1">
                ...
                {sectionForm.thumbnail_url && <Button variant="ghost" size="icon" className="h-10 w-10" onClick={() => setSectionForm(p => ({ ...p, thumbnail_url: '' }))}><X className="h-4 w-4" /></Button>}
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label>الترتيب</Label><Input type="number" value={sectionForm.display_order} onChange={(e) => setSectionForm((p) => ({ ...p, display_order: Number(e.target.value) }))} /></div>
              <div className="flex items-center gap-2 pt-6"><Switch checked={sectionForm.is_active} onCheckedChange={(v) => setSectionForm((p) => ({ ...p, is_active: v }))} /><Label>نشط</Label></div>
            </div>
            <Button className="w-full" disabled={!sectionForm.title_ar || saveSectionMutation.isPending} onClick={() => saveSectionMutation.mutate({ ...sectionForm, id: editingSection?.id })}>
              {saveSectionMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin ml-1" /> : null} حفظ
            </Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={videoDialog} onOpenChange={setVideoDialog}>
        <DialogContent dir="rtl" className="max-w-md">
          <DialogHeader><DialogTitle>{editingVideo ? 'تعديل الفيديو' : 'إضافة فيديو'}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div>
            ...
            {videoForm.video_url && <div className="aspect-[9/16] max-h-48 rounded-lg overflow-hidden bg-muted"><video src={videoForm.video_url} className="w-full h-full object-contain" controls muted /></div>}
            <div className="grid grid-cols-2 gap-3">
              <div><Label>الترتيب</Label><Input type="number" value={videoForm.display_order} onChange={(e) => setVideoForm((p) => ({ ...p, display_order: Number(e.target.value) }))} /></div>
              <div className="flex items-center gap-2 pt-6"><Switch checked={videoForm.is_active} onCheckedChange={(v) => setVideoForm((p) => ({ ...p, is_active: v }))} /><Label>نشط</Label></div>
            </div>
            <Button className="w-full" disabled={!videoForm.video_url || !selectedSectionId || saveVideoMutation.isPending} onClick={() => saveVideoMutation.mutate({ ...videoForm, section_id: selectedSectionId!, id: editingVideo?.id })}>
              {saveVideoMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin ml-1" /> : null} حفظ
            </Button>
          </div>
        </DialogContent>
      </Dialog>
