...
import { ArrowRight, Plus, Trash2, Save, Settings, Gift, Ticket, Eye } from "lucide-react";
...
export default function AdminMysteryCase() {
  ...
  const saveSettings = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["admin-mystery-settings"] });
    },
  ...
  const openEditReward = (r: any) => {
    ...
    setRewardForm({
      ...
      ticket_reward_amount: r.ticket_reward_amount || 0,
  ...
  const saveReward = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        drop_chance: rewardForm.drop_chance,
        image_url: rewardForm.image_url || null,
        product_id: rewardForm.product_id || null,
        ticket_reward_amount: rewardForm.ticket_reward_amount,
        display_only: rewardForm.display_only,
    ...
    <div className="min-h-screen bg-background p-4" dir="rtl">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold">🎰 إدارة صندوق الغموض</h1>
            <p className="text-sm text-muted-foreground">إعداد اللعبة والجوائز</p>
          </div>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  type="number"
                  min={1}
                  value={settingsForm.tickets_per_spin}
                  onChange={(e) => setSettingsForm({ ...settingsForm, tickets_per_spin: parseInt(e.target.value) || 1 })}
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  min={0}
                  value={settingsForm.spin_cooldown_seconds}
                  onChange={(e) => setSettingsForm({ ...settingsForm, spin_cooldown_seconds: parseInt(e.target.value) || 0 })}
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  min={2000}
                  value={settingsForm.animation_duration_ms}
                  onChange={(e) => setSettingsForm({ ...settingsForm, animation_duration_ms: parseInt(e.target.value) || 5000 })}
                />
              </div>
            </div>
            <div className="flex flex-wrap gap-4">
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={settingsForm.game_enabled}
                  onChange={(e) => setSettingsForm({ ...settingsForm, game_enabled: e.target.checked })}
                />
                تفعيل اللعبة
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={settingsForm.spin_sound_enabled}
                  onChange={(e) => setSettingsForm({ ...settingsForm, spin_sound_enabled: e.target.checked })}
                />
                أصوات اللعبة
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={settingsForm.daily_free_spin}
                  onChange={(e) => setSettingsForm({ ...settingsForm, daily_free_spin: e.target.checked })}
                />
                لفة مجانية يومية
              </label>
            </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base">
              <Gift className="h-4 w-4" /> إدارة الجوائز
              <span className="text-xs text-muted-foreground mr-auto">
                إجمالي الاحتمالات: {totalChance.toFixed(1)}%
              </span>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {/* Reward Form */}
            <div className="p-4 rounded-lg border border-border/50 space-y-3 bg-muted/5">
              <h4 className="text-sm font-bold">{editReward ? "تعديل جائزة" : "إضافة جائزة جديدة"}</h4>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-muted-foreground">الاسم (عربي)</label>
                  <Input
                    value={rewardForm.name_ar}
                    onChange={(e) => setRewardForm({ ...rewardForm, name_ar: e.target.value })}
                ...
                <div>
                  ...
                  <Input
                    ...
                    min={0}
                    step={0.01}
                    value={rewardForm.drop_chance}
                    onChange={(e) => setRewardForm({ ...rewardForm, drop_chance: parseFloat(e.target.value) || 0 })}
                  />
                </div>
                ...
                {rewardForm.reward_type === "product" && (
                  <div className="col-span-2">
                    ...
                    <select
                      ...
                      onChange={(e) => setRewardForm({ ...rewardForm, product_id: e.target.value })}
                    >
                ...
                {rewardForm.reward_type === "tickets" && (
                  <div>
                    ...
                    <Input
                      type="number"
                      min={1}
                      value={rewardForm.ticket_reward_amount}
                      onChange={(e) => setRewardForm({ ...rewardForm, ticket_reward_amount: parseInt(e.target.value) || 0 })}
                    />
                  </div>
                )}
              </div>
              <div className="flex flex-wrap gap-4">
                <label className="flex items-center gap-2 text-sm">
                  <input
                    type="checkbox"
                    checked={rewardForm.display_only}
                    onChange={(e) => setRewardForm({ ...rewardForm, display_only: e.target.checked })}
                  />
              ...
              <div className="flex gap-2">
                ...
                {editReward && (
                  <Button onClick={resetRewardForm} variant="ghost" size="sm">إلغاء</Button>
                )}
              </div>
            </div>
            ...
            <div className="space-y-2">
              {rewards.map((r: any) => {
                ...
                return (
                  <div
                    key={r.id}
                    className="flex items-center gap-3 p-3 rounded-lg border border-border/50 hover:bg-muted/5"
                  >
                    {r.image_url ? (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        <span
                          ...
                          style={{ background: `${rarityInfo?.color}22`, color: rarityInfo?.color }}
                        >
                          {rarityInfo?.label}
                        </span>
                    ...
                    <div className="flex gap-1">
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openEditReward(r)}>
                        ✏️
                      </Button>
                      <Button
                        variant="ghost"
              ...
              {rewards.length === 0 && (
                <p className="text-center text-sm text-muted-foreground py-8">لم تتم إضافة جوائز بعد</p>
              )}
            </div>
          </CardContent>
        </Card>
