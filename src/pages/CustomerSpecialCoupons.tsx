      ...
      <main className="flex-1 px-4 py-3 space-y-5">
        ...
        {showMerchantManager && isMerchant && merchantApp && (
          <div className="rounded-2xl border border-primary/20 bg-card/50 backdrop-blur-sm overflow-hidden">
            <MerchantDiscountsManager merchantId={merchantApp.id} merchantName={merchantApp.display_name || "متجري"} />
          </div>
        )}
          ...
          {selectedDiscount && (() => {
            ...
            return (
              <div className="flex flex-col h-full">
                <div className="relative bg-gradient-to-br from-primary to-primary/60 px-5 pt-7 pb-5">
                  <div className="absolute top-2.5 left-1/2 -translate-x-1/2 w-10 h-1 rounded-full bg-primary-foreground/30" />
                  <div className="relative">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="flex-1">
                        <Badge className="bg-primary-foreground/20 text-primary-foreground border-0 text-[9px] mb-0.5">
                    ...
                    <div className="text-4xl font-black text-primary-foreground drop-shadow-lg">
                      {getDiscountDisplay(selectedDiscount)}
                    </div>
                    {selectedDiscount.merchant_store_name && (
                      <div className="flex items-center gap-1.5 text-primary-foreground/80 text-[11px] mt-2">
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-3">
                  {selectedDiscount.description_ar && (
                    <p className="text-xs text-muted-foreground leading-relaxed">{selectedDiscount.description_ar}</p>
                  )}
                  {selectedDiscount.min_purchase_amount > 0 && (
                    <div className="rounded-xl bg-muted/30 border border-border/30 p-3 flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-[11px] font-bold text-foreground">الحد الأدنى</p>
                        <p className="text-[10px] text-muted-foreground">{selectedDiscount.min_purchase_amount.toLocaleString()} د.ع</p>
                      </div>
                    </div>
                  )}
                  {selectedDiscount.gift_description && (
                    <div className="rounded-xl bg-muted/30 border border-border/30 p-3 flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-[11px] font-bold text-foreground">الهدية</p>
                        <p className="text-[10px] text-muted-foreground">{selectedDiscount.gift_description}</p>
                      </div>
                    </div>
                  )}
                  {selectedDiscount.valid_until && (
                    <div className="rounded-xl bg-muted/30 border border-border/30 p-3 flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-[11px] font-bold text-foreground">صالح حتى</p>
                        <p className="text-[10px] text-muted-foreground">{format(new Date(selectedDiscount.valid_until), "d MMMM yyyy", { locale: ar })}</p>
                      </div>
                    </div>
                  )}
                </div>
