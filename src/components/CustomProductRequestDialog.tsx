...
import { Package, Loader2, X, Ship, Plane, Globe } from 'lucide-react';
    ...
    <Dialog open={open} onOpenChange={(isOpen) => {
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            أدخل تفاصيل المنتج الذي ترغب في طلبه وسنتواصل معك لتحديد السعر
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            {/* Country & Shipping Type Selection - Required */}
            <div className="grid grid-cols-2 gap-4 p-4 bg-muted/30 rounded-lg border border-primary/20">
              ...
              <div className="space-y-2">
                <Label className="flex items-center gap-2 text-sm font-bold">
                  {shippingType === 'sea' ? <Ship className="h-4 w-4 text-primary" /> : <Plane className="h-4 w-4 text-primary" />}
                  نوع الشحن *
                </Label>
