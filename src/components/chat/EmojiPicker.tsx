...
export default function EmojiPicker({ onSelectEmoji, className }: EmojiPickerProps) {
  ...
  return (
    <div 
      className={cn("bg-card rounded-2xl overflow-hidden shadow-xl border", className)}
              ...
              <div className="flex gap-1">
                {recentEmojis.map((emoji) => (
                  <button
                    ...
                    onPointerUp={(e) => {
                      e.preventDefault();
                      handleEmojiClick(e as any, emoji.code);
                    }}
                    className="w-9 h-9 p-1 rounded-lg hover:bg-primary/10 active:scale-90 transition-all duration-150 flex items-center justify-center"
                  >
                    <img
                      src={emoji.src}
                      alt={emoji.alt}
            ...
            <div className="grid grid-cols-8 gap-1">
              {WECHAT_EMOJIS.map((emoji) => (
                <button
                  ...
                  className={cn(
                    "aspect-square p-1 rounded-lg hover:bg-primary/10 active:scale-90 transition-all duration-150 flex items-center justify-center",
