    ...
    rollupOptions: {
      output: {
        manualChunks: (id) => {
          ...
          // Keep third-party libraries in one vendor graph. Splitting packages
          // like Recharts and QR libraries into standalone manual chunks caused
          // Safari/iPad production TDZ crashes before React could mount, leaving
          // users stuck on the static loading fallback.
          if (
